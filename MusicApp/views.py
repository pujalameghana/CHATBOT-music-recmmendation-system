...
def trainModel(query):
    ...
    answers = []
    details = []
    con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'ChatbotMusic',charset='utf8')
    with con:
        cur = con.cursor()
        ...
        with con:    
            ...
            for row in rows:
                if row[0] == username:
                    ...
                    break
        if status == "none":
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'ChatbotMusic',charset='utf8')
